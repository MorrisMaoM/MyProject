...
<%
	...
	List<Order_TableVO> list = order_tableSvc.getAllComplete_Order(rs_id);
	if (request.getParameter("rs_id") != null) {
		//		System.out.println("1");
		rs_id = request.getParameter("rs_id");
		System.out.println("rs_id" + rs_id);
		list = order_tableSvc.getAllComplete_Order(request.getParameter("rs_id"));
	} else {
		//		System.out.println("2");
		//		System.out.println("rs_id"+(String)session.getAttribute("rs_id"));
		rs_id = (String) session.getAttribute("rs_id");
		list = order_tableSvc.getAllComplete_Order((String) session.getAttribute("rs_id"));
	}
...
<!-- 						height="90" width="90"></td> -->
					...
					<td><c:choose>
							<c:when test="${order_tableVO.order_status==2}">
								<a href="#">
									<button type="button" class="button_THREE_TYPE" id="ff">完成訂單</button>
								</a>
							</c:when>
							<c:otherwise>
								<button type="button" class="btn_modal"
									id="${order_tableVO.order_id}">完成訂單</button>
					...
					$.ajax({
						...
						success : function(result) {
							if (result.update_ok == "success") {
								...
								that.parent().parent().children(".order_time")
										.first().text("已完成")
								that.parent().parent().children(".cancel_time")
										.first().text("無")
