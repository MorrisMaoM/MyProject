...
<%
	...
	List<Order_TableVO> list = order_tableSvc.getAllRs_id(rs_id); //正式時打開使用
...
<%-- 			?order_status=${order_tableVO.order_status} --%>	
		...
		<br>
		<table id="type" style="font-size:14px; font-weight: 900; border: 1px solid #272727 ;">
			<tr>
				<th>餐廳編號</th>
				<th >訂單編號</th>
				<th>消費者信箱</th>
...
<!-- 				<th>餐廳座位狀態</th> -->
		...
		<c:forEach var="order_tableVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
				<tr>
					...
					<td id="${order_tableVO.order_id}" class="status_text">
						${order_tableVO.order_status==0?"取消":""}
						${order_tableVO.order_status==1?"成立":""}
						${order_tableVO.order_status==2?"已完成":""}
					</td>
						<td class ="order_time">
							<c:choose>
								<c:when test="${order_tableVO.order_status==1 }">
									<fmt:formatDate value="${order_tableVO.order_time}" pattern="yyyy-MM-dd HH:mm"/>
								</c:when>
					...
					<td class ="cancel_time">
						<c:choose>
								<c:when test="${order_tableVO.order_status == 0 }">
									<fmt:formatDate value="${order_tableVO.order_cancel_time}" pattern="yyyy-MM-dd HH:mm"/>
								</c:when>
								<c:when test="${order_tableVO.order_status == 2 }">
									無
								</c:when>
								<c:otherwise >
									無
...
<%-- 					<td>${order_tableVO.rs_table_status}</td> --%>
					...
					<c:choose>
						...
						<c:when test="${order_tableVO.order_status==2}">
							<a href="#">
		   						<button type="button" class="button_THREE_TYPE" id="fff">已完成報到</button>
		   					</a>
						</c:when>
						<c:otherwise>
							<button type="button" class="btn_modal" id="${order_tableVO.order_id}">完成訂單</button>
...
// 	 			});
    		...
    		$.ajax({
    			...
    			success : function(result) {
    				...
    				that.parent().parent().children(".order_time").first().text("已完成")
    				that.parent().parent().children(".cancel_time").first().text("無")
...
// 	 		$('#order_tableVO.order_id').mousepress(function(e)) {
// 	 			if(e.mousedown =='left') {
...
// 	 				url:"/order_table/controller/Order_TableServlet_front.java"
